export const translations = {
    en: {
        // Header
        driverPanel: "Driver Panel",
        logout: "Logout",
        idle: "Idle",
        trip: "Trip",

        // Tabs
        activeTrip: "Active Trip",
        tripHistory: "Trip History",

        // Active Trip
        startNewTrip: "Start New Trip",
        starting: "Starting...",
        processing: "Processing...",
        tripCompleted: "Trip Completed!",

        // States
        // States (mapped to DB values)
        "Exit Factory": "Exit Factory",
        "Arrival at Warehouse": "Arrival at Warehouse",
        "Exit Warehouse": "Exit Warehouse",
        "Arrival at Factory": "Arrival at Factory",
        EXIT_FACTORY: "Exit Factory",
        ARRIVE_WAREHOUSE: "Arrival at Warehouse",
        EXIT_WAREHOUSE: "Exit Warehouse",
        ARRIVE_FACTORY: "Arrival at Factory",

        // Actions
        logExitFactory: "Log Exit Factory",
        logArriveWarehouse: "Log Arrive Warehouse",
        logExitWarehouse: "Log Exit Warehouse",
        logArriveFactoryEnd: "Log Arrive Factory (End Trip)",
        goToAnotherWarehouse: "Go to Another Warehouse",
        returnToFactory: "Return to Factory",
        warehouseExitQuestion: "You have exited the warehouse.",
        whereGoingNext: "Where are you going next?",

        // Timeline
        tripTimeline: "Trip Timeline",
        steps: "Steps",
        next: "Next",

        // Months
        january: "January",
        february: "February",
        march: "March",
        april: "April",
        may: "May",
        june: "June",
        july: "July",
        august: "August",
        september: "September",
        october: "October",
        november: "November",
        december: "December",

        // Trip History
        loadingTrips: "Loading trips...",
        noTripsFound: "No trips found",
        noTripsMessage: "You haven't completed any trips in",
        viewDetails: "View Details",
        hideDetails: "Hide Details",
        timeline: "Timeline",
        warehouses: "Warehouses",
        warehouse: "Warehouse",
        duration: "Duration",
        completed: "Completed",

        // Errors & Warnings
        locationDenied: "Location permission denied. Please enable GPS.",
        locationUnavailable: "Location unavailable. Check GPS signal.",
        failedToLogState: "Failed to log state. Ensure GPS is enabled.",
        locationWarning: "⚠️ Location permission is required. Please check your browser settings.",
        failedToStartTrip: "Failed to start trip. Please try again.",

        // Misc
        pinnedLocation: "Pinned Location",
        loadingTripState: "Loading Trip State...",

        // ─── Admin Dashboard ───
        adminDashboard: "Admin Dashboard",
        trips: "Trips",
        cars: "Cars",
        drivers: "Drivers",
        allDrivers: "All Drivers",
        exportBtn: "Export",
        addDriver: "Add Driver",
        addCar: "Add Car",
        editTrip: "Edit Trip",
        editDriver: "Edit Driver",
        newDriver: "New Driver",
        saveChanges: "Save Changes",
        saveCar: "Save Car",
        saveSettings: "Save Settings",
        createDriver: "Create Driver",
        updateDriver: "Update Driver",
        details: "Details",
        startTime: "Start Time",
        statusLabel: "Status",
        active: "Active",
        inProgress: "In Progress",
        driverLabel: "Driver",
        carLabel: "Car",
        username: "Username",
        passwordLabel: "Password",
        passwordKeep: "(Leave blank to keep)",
        assignCar: "Assign Car",
        noCarAssigned: "-- No Car Assigned --",
        noCar: "No Car",
        plateNumber: "Plate Number",
        modelDescription: "Model / Description",
        companySettings: "Company Settings",
        companyNameLabel: "Company Name",
        changePassword: "Change Password",
        newPassword: "New password",
        startDate: "Start Date",
        tripLogsEvents: "Trip Logs / Events",
        timeLabel: "Time",
        locationAddress: "Location / Address",
        enterLocation: "Enter location",
        actions: "Actions",
        idLabel: "ID",
        assignedCar: "Assigned Car",
        plate: "Plate",
        model: "Model",
        noTripsFoundAdmin: "No trips found.",
        noCarsFound: "No cars found. Add one!",
        noLogsYet: "No logs recorded yet.",
        logo: "Logo",
        currentLogo: "Current Logo:",
        languageLabel: "Language",
        confirmDeleteTrip: "Are you sure you want to delete this trip/log?",
        confirmDeleteDriver: "Are you sure you want to delete this driver? All their trips will be deleted.",
        confirmDeleteCar: "Are you sure?",
        tripUpdated: "Trip updated successfully!",
        tripDeleted: "Trip deleted successfully.",
        driverCreated: "Driver created successfully!",
        driverUpdated: "Driver updated successfully!",
        driverDeleted: "Driver deleted.",
        carAdded: "Car added successfully!",
        carDeleted: "Car deleted.",
        passwordChanged: "Password changed successfully.",
        settingsUpdated: "Settings updated successfully!",
        failedUpdateTrip: "Failed to update trip.",
        failedDeleteTrip: "Failed to delete trip.",
        failedSaveDriver: "Failed to save driver. Username might be taken.",
        failedDeleteDriver: "Failed to delete driver.",
        failedAddCar: "Failed to add car. Plate might already exist.",
        failedSaveCar: "Failed to save car. Plate might already exist.",
        failedDeleteCar: "Failed to delete car. It might be assigned to a driver.",
        failedChangePassword: "Failed to change password.",
        failedUpdateSettings: "Failed to update settings.",
        timelineEvents: "Timeline Events",
        tripDetails: "Trip Details",
        fromDate: "From Date",
        toDate: "To Date",
        clear: "Clear",
        unknown: "Unknown",
        na: "N/A",
        addEvent: "Add New Event",
        removeEvent: "Remove Event",
        stateLabel: "Event Type",
        viewOnMap: "View on Map",
        fuelCapacity: "Fuel Capacity (Liters)",
        editCar: "Edit Car",

        // ─── Analytics Dashboard ───
        dashboard: "Dashboard",
        monthlyAnalytics: "Monthly Analytics",
        totalTrips: "Total Trips",
        activeTrips: "Active Trips",
        completedTrips: "Completed Trips",
        totalDrivers: "Total Drivers",
        totalCars: "Total Cars",
        tripsToday: "Trips Today",
        tripsThisMonth: "Trips This Month",
        tripsOverTime: "Trips Over Time (Last 30 Days)",
        tripsPerDriver: "Trips Per Driver",
        tripStatusBreakdown: "Trip Status",
        avgTripDuration: "Average Trip Duration",
        recentActivity: "Recent Activity",
        inactiveDrivers: "Inactive Drivers (48h)",
        inactive48h: "No activity in last 48 hours",
        noInactiveDrivers: "All drivers were active recently",
        hours: "hours",
        minutes: "min",
        noData: "No data available",
        overallAvg: "Overall Average",
        completed: "Completed",
        tripCount: "Trips",
        cityDestinations: "City Destinations Per Driver",
        tripDurationByCity: "Trip Duration By City",
        city: "City",
        duration: "Duration",
        driver: "Driver",
        selectDriver: "Select Driver",
        allDrivers: "All Drivers",
        tripId: "Trip #",
        carPlate: "Car Plate",
        arrivalTime: "Arrived Time",
        vehicleStatus: "Vehicle Status",
        atFactory: "Cars at Factory",
        readyToDepart: "Ready to Depart",
        returnedToFactory: "Returned to Factory",
        atWarehouse: "Cars at Warehouse",
        outbound: "Loaded Cars (Outbound)",
        inbound: "Returning Cars (Inbound)",
        tripTimeTracking: "Trip Time Tracking",
        departureTime: "Departure Time",
        returnTime: "Return Time",
        waitingTime: "Waiting Time",
        timeToWarehouse: "Time to Warehouse",
        timeToReturn: "Time to Return",
        totalTripTime: "Total Trip Time",
        status: "Status",
        viewDetails: "View Details",
        noVehiclesInState: "No vehicles currently in this state.",
        hideDetails: "Hide Details",
        timeMetrics: "Time Metrics",
        timeAtFactory: "Time at Factory",
        excelView: "Excel View",
        sequence: "Seq",
        exitFactoryShort: "Exit Factory",
        arriveWarehouseShort: "Arrive Warehouse",
        tripTimeToWarehouse: "Trip Time to Warehouse",
        exitWarehouseShort: "Exit Warehouse",
        arriveFactoryShort: "Arrive Factory",
        returnTimeToFactory: "Return Time to Factory",
        waitingTimeAtWarehouse: "Waiting Time",
        waitingReason: "Waiting Reason",
        totalActualTime: "Total Actual Time",
        totalEstimatedTime: "Total Estimated Time",
        difference: "Difference",
        delay: "Delay",
        carPlate: "Car Plate",
        city: "City",
        logFuelRefill: "Log Fuel Refill",
        fuelAmountLiters: "Fuel Amount (Liters)",
        fuelIndicatorPhoto: "Fuel Indicator Photo",
        petrolMachinePhoto: "Petrol Machine Photo",
        uploadPhoto: "Tap to Take/Upload Photo",
        submitRefill: "Submit Refill",
        fuelLoggedSuccess: "Fuel refill logged successfully!",
        fuelRequiredFields: "Please fill the amount and take both photos.",
        failedLogFuel: "Failed to log fuel refill. Check your connection/GPS.",
        fuelMonitoring: "Fuel Monitoring",
        noActiveTripForFuel: "You need an active trip to log a fuel refill. Please start a trip first.",
        distanceKm: "Distance (km)",
        expectedConsumption: "Expected (L)",
        actualRefills: "Actual (L)",
        viewPhotos: "View Photos",
        indicator: "Indicator",
        machine: "Machine",
        totalDistance: "Total Distance (km)",
        totalEstimated: "Total Estimated (L)",
        totalActual: "Total Actual (L)",
        fuelHistory: "Fuel Refill History",
        viewHistory: "View History",
        driverName: "Driver Name",
        avgConsumption: "Avg Consumption (L/100km)",
        discrepancy: "Discrepancy (L)",
    },
    ar: {
        // Header
        driverPanel: "لوحة السائق",
        logout: "تسجيل الخروج",
        idle: "خامل",
        trip: "رحلة",

        // Tabs
        activeTrip: "الرحلة النشطة",
        tripHistory: "سجل الرحلات",

        // Active Trip
        startNewTrip: "بدء رحلة جديدة",
        starting: "جاري البدء...",
        processing: "جاري المعالجة...",
        tripCompleted: "اكتملت الرحلة!",

        // States
        exitFactory: "الخروج من المصنع",
        arriveWarehouse: "الوصول إلى المستودع",
        exitWarehouse: "الخروج من المستودع",
        arriveFactory: "الوصول إلى المصنع",
        // Direct DB mappings
        "Exit Factory": "الخروج من المصنع",
        "Arrival at Warehouse": "الوصول إلى المستودع",
        "Exit Warehouse": "الخروج من المستودع",
        "Arrival at Factory": "الوصول إلى المصنع",
        EXIT_FACTORY: "خروج من المصنع",
        ARRIVE_WAREHOUSE: "وصول إلى المستودع",
        EXIT_WAREHOUSE: "خروج من المستودع",
        ARRIVE_FACTORY: "وصول إلى المصنع",

        // Actions
        logExitFactory: "تسجيل الخروج من المصنع",
        logArriveWarehouse: "تسجيل الوصول للمستودع",
        logExitWarehouse: "تسجيل الخروج من المستودع",
        logArriveFactoryEnd: "تسجيل الوصول للمصنع (إنهاء الرحلة)",
        goToAnotherWarehouse: "الذهاب إلى مستودع آخر",
        returnToFactory: "العودة إلى المصنع",
        warehouseExitQuestion: "لقد خرجت من المستودع.",
        whereGoingNext: "إلى أين تتجه الآن؟",

        // Timeline
        tripTimeline: "الجدول الزمني للرحلة",
        steps: "خطوات",
        next: "التالي",

        // Months
        january: "يناير",
        february: "فبراير",
        march: "مارس",
        april: "أبريل",
        may: "مايو",
        june: "يونيو",
        july: "يوليو",
        august: "أغسطس",
        september: "سبتمبر",
        october: "أكتوبر",
        november: "نوفمبر",
        december: "ديسمبر",

        // Trip History
        loadingTrips: "جاري تحميل الرحلات...",
        noTripsFound: "لا توجد رحلات",
        noTripsMessage: "لم تكمل أي رحلات في",
        viewDetails: "عرض التفاصيل",
        hideDetails: "إخفاء التفاصيل",
        timeline: "الجدول الزمني",
        warehouses: "مستودعات",
        warehouse: "مستودع",
        duration: "المدة",
        completed: "مكتملة",

        // Errors & Warnings
        locationDenied: "تم رفض إذن الموقع. يرجى تفعيل GPS.",
        locationUnavailable: "الموقع غير متاح. تحقق من إشارة GPS.",
        failedToLogState: "فشل تسجيل الحالة. تأكد من تفعيل GPS.",
        locationWarning: "⚠️ إذن الموقع مطلوب. يرجى التحقق من إعدادات المتصفح.",
        failedToStartTrip: "فشل بدء الرحلة. يرجى المحاولة مرة أخرى.",

        // Misc
        pinnedLocation: "موقع محدد",
        loadingTripState: "جاري تحميل حالة الرحلة...",

        // ─── Admin Dashboard ───
        adminDashboard: "لوحة التحكم",
        trips: "الرحلات",
        cars: "السيارات",
        drivers: "السائقين",
        allDrivers: "كل السائقين",
        exportBtn: "تصدير",
        addDriver: "إضافة سائق",
        addCar: "إضافة سيارة",
        editTrip: "تعديل الرحلة",
        editDriver: "تعديل السائق",
        newDriver: "سائق جديد",
        saveChanges: "حفظ التغييرات",
        saveCar: "حفظ السيارة",
        saveSettings: "حفظ الإعدادات",
        createDriver: "إنشاء سائق",
        updateDriver: "تحديث السائق",
        details: "التفاصيل",
        startTime: "وقت البدء",
        statusLabel: "الحالة",
        active: "نشط",
        inProgress: "قيد التنفيذ",
        driverLabel: "السائق",
        carLabel: "السيارة",
        username: "اسم المستخدم",
        passwordLabel: "كلمة المرور",
        passwordKeep: "(اتركه فارغاً للإبقاء)",
        assignCar: "تعيين سيارة",
        noCarAssigned: "-- لا توجد سيارة --",
        noCar: "لا توجد سيارة",
        plateNumber: "رقم اللوحة",
        modelDescription: "الموديل / الوصف",
        companySettings: "إعدادات الشركة",
        companyNameLabel: "اسم الشركة",
        changePassword: "تغيير كلمة المرور",
        newPassword: "كلمة المرور الجديدة",
        startDate: "تاريخ البدء",
        tripLogsEvents: "سجلات الرحلة",
        timeLabel: "الوقت",
        locationAddress: "العنوان / الموقع",
        enterLocation: "أدخل الموقع",
        actions: "الإجراءات",
        idLabel: "رقم",
        assignedCar: "السيارة المخصصة",
        plate: "اللوحة",
        model: "الموديل",
        noTripsFoundAdmin: "لا توجد رحلات.",
        noCarsFound: "لا توجد سيارات. أضف واحدة!",
        noLogsYet: "لم يتم تسجيل أي سجلات بعد.",
        logo: "الشعار",
        currentLogo: "الشعار الحالي:",
        languageLabel: "اللغة",
        confirmDeleteTrip: "هل أنت متأكد من حذف هذه الرحلة؟",
        confirmDeleteDriver: "هل أنت متأكد من حذف هذا السائق؟ سيتم حذف جميع رحلاته.",
        confirmDeleteCar: "هل أنت متأكد؟",
        tripUpdated: "تم تحديث الرحلة بنجاح!",
        tripDeleted: "تم حذف الرحلة بنجاح.",
        driverCreated: "تم إنشاء السائق بنجاح!",
        driverUpdated: "تم تحديث السائق بنجاح!",
        driverDeleted: "تم حذف السائق.",
        carAdded: "تمت إضافة السيارة بنجاح!",
        carDeleted: "تم حذف السيارة.",
        passwordChanged: "تم تغيير كلمة المرور بنجاح.",
        settingsUpdated: "تم تحديث الإعدادات بنجاح!",
        failedUpdateTrip: "فشل تحديث الرحلة.",
        failedDeleteTrip: "فشل حذف الرحلة.",
        failedSaveDriver: "فشل حفظ السائق. قد يكون اسم المستخدم مأخوذاً.",
        failedDeleteDriver: "فشل حذف السائق.",
        failedAddCar: "فشل إضافة السيارة. قد تكون اللوحة موجودة.",
        failedSaveCar: "فشل حفظ السيارة. قد تكون اللوحة موجودة.",
        failedDeleteCar: "فشل حذف السيارة. قد تكون مخصصة لسائق.",
        failedChangePassword: "فشل تغيير كلمة المرور.",
        failedUpdateSettings: "فشل تحديث الإعدادات.",
        timelineEvents: "الأحداث الزمنية",
        tripDetails: "تفاصيل الرحلة",
        fromDate: "من تاريخ",
        toDate: "إلى تاريخ",
        clear: "مسح",
        unknown: "غير معروف",
        na: "غير متوفر",
        addEvent: "إضافة حدث جديد",
        removeEvent: "حذف الحدث",
        stateLabel: "نوع الحدث",
        viewOnMap: "عرض على الخريطة",
        fuelCapacity: "سعة خزان الوقود (لتر)",
        editCar: "تعديل السيارة",

        // ─── Analytics Dashboard ───
        dashboard: "لوحة المعلومات",
        monthlyAnalytics: "التحليلات الشهرية",
        totalTrips: "إجمالي الرحلات",
        activeTrips: "الرحلات النشطة",
        completedTrips: "الرحلات المكتملة",
        totalDrivers: "إجمالي السائقين",
        totalCars: "إجمالي السيارات",
        tripsToday: "رحلات اليوم",
        tripsThisMonth: "رحلات هذا الشهر",
        tripsOverTime: "الرحلات خلال آخر 30 يوم",
        tripsPerDriver: "الرحلات لكل سائق",
        tripStatusBreakdown: "حالة الرحلات",
        avgTripDuration: "متوسط مدة الرحلة",
        recentActivity: "النشاط الأخير",
        inactiveDrivers: "السائقين غير النشطين (48 ساعة)",
        inactive48h: "لا يوجد نشاط في آخر 48 ساعة",
        noInactiveDrivers: "جميع السائقين نشطون مؤخراً",
        hours: "ساعات",
        minutes: "دقيقة",
        noData: "لا توجد بيانات",
        overallAvg: "المعدل العام",
        completed: "مكتملة",
        tripCount: "رحلات",
        cityDestinations: "المدن المقصودة لكل سائق",
        tripDurationByCity: "مدة الرحلة حسب المدينة",
        city: "المدينة",
        duration: "المدة",
        driver: "السائق",
        selectDriver: "اختر سائق",
        allDrivers: "كل السائقين",
        tripId: "رحلة #",
        carPlate: "رقم اللوحة",
        arrivalTime: "وقت الوصول",
        vehicleStatus: "حالة المركبات",
        atFactory: "مركبات في المصنع",
        readyToDepart: "جاهزة للانطلاق",
        returnedToFactory: "عادت للمصنع",
        atWarehouse: "مركبات في المستودع",
        outbound: "مركبات محملة (مغادرة)",
        inbound: "مركبات عائدة",
        tripTimeTracking: "تتبع وقت الرحلات",
        departureTime: "وقت المغادرة",
        returnTime: "وقت العودة",
        waitingTime: "وقت الانتظار",
        timeToWarehouse: "الوقت إلى المستودع",
        timeToReturn: "وقت العودة للمصنع",
        totalTripTime: "إجمالي وقت الرحلة",
        status: "الحالة",
        viewDetails: "عرض التفاصيل",
        noVehiclesInState: "لا توجد مركبات في هذه الحالة حالياً.",
        hideDetails: "إخفاء التفاصيل",
        timeMetrics: "مقاييس الوقت",
        timeAtFactory: "الوقت في المصنع",
        excelView: "عرض إكسل",
        sequence: "التسلسل",
        exitFactoryShort: "التحرك من المصنع",
        arriveWarehouseShort: "الوصول للمستودع",
        tripTimeToWarehouse: "وقت الرحلة إلى المستودع",
        exitWarehouseShort: "التحرك من المستودع",
        arriveFactoryShort: "الوصول للمصنع",
        returnTimeToFactory: "وقت الرجوع للمصنع",
        waitingTimeAtWarehouse: "وقت الانتظار بالمستودع",
        waitingReason: "سبب الانتظار بالمستودع",
        totalActualTime: "إجمالي الوقت الفعلي",
        totalEstimatedTime: "إجمالي الوقت المقدر",
        difference: "الفرق",
        delay: "التأخير",
        carPlate: "رقم اللوحة",
        city: "المدينة",
        logFuelRefill: "تسجيل وقود",
        fuelAmountLiters: "كمية الوقود (لتر)",
        fuelIndicatorPhoto: "صورة مؤشر الوقود",
        petrolMachinePhoto: "صورة ماكينة المحطة",
        uploadPhoto: "اضغط للالتقاط/الرفع",
        submitRefill: "إرسال البيانات",
        fuelLoggedSuccess: "تم تسجيل الوقود بنجاح!",
        fuelRequiredFields: "يرجى إدخال الكمية والتقاط الصورتين.",
        failedLogFuel: "فشل تسجيل الوقود. تحقق من الاتصال/الموقع.",
        fuelMonitoring: "مراقبة الوقود",
        noActiveTripForFuel: "يجب أن تكون في رحلة نشطة لتسجيل الوقود. ابدأ رحلة أولاً.",
        distanceKm: "المسافة (كم)",
        expectedConsumption: "المتوقع (لتر)",
        actualRefills: "الفعلي (لتر)",
        viewPhotos: "عرض الصور",
        indicator: "المؤشر",
        machine: "الماكينة",
        totalDistance: "إجمالي المسافة (كم)",
        totalEstimated: "إجمالي الاستهلاك المقدر (لتر)",
        totalActual: "إجمالي التعبئة الفعلية (لتر)",
        fuelHistory: "سجل تعبئة الوقود",
        viewHistory: "عرض السجل",
        driverName: "اسم السائق",
        avgConsumption: "متوسط الاستهلاك (لتر/100كم)",
        discrepancy: "الفرق (لتر)",
    },
    ur: {
        // Header
        driverPanel: "ڈرائیور پینل",
        logout: "لاگ آؤٹ",
        idle: "بیکار",
        trip: "واپس",

        // Tabs
        activeTrip: "فعال سفر",
        tripHistory: "سفر کی تاریخ",

        // Active Trip
        startNewTrip: "نیا سفر شروع کریں",
        starting: "شروع ہو رہا ہے...",
        processing: "کارروائی ہو رہی ہے...",
        tripCompleted: "سفر مکمل!",

        // States
        exitFactory: "فیکٹری سے خروج",
        arriveWarehouse: "گودام آمد",
        exitWarehouse: "گودام سے خروج",
        arriveFactory: "فیکٹری آمد",
        // Direct DB mappings
        "Exit Factory": "فیکٹری سے خروج",
        "Arrival at Warehouse": "گودام آمد",
        "Exit Warehouse": "گودام سے خروج",
        "Arrival at Factory": "فیکٹری آمد",
        EXIT_FACTORY: "فیکٹری سے خروج",
        ARRIVE_WAREHOUSE: "گودام آمد",
        EXIT_WAREHOUSE: "گودام سے خروج",
        ARRIVE_FACTORY: "فیکٹری آمد",

        // Actions
        logExitFactory: "لاگ فیکٹری سے خروج",
        logArriveWarehouse: "لاگ گودام آمد",
        logExitWarehouse: "لاگ گودام سے خروج",
        logArriveFactoryEnd: "لاگ فیکٹری آمد (سفر کا اختتام)",
        goToAnotherWarehouse: "دوسرے گودام پر جائیں",
        returnToFactory: "فیکٹری واپس جائیں",
        warehouseExitQuestion: "آپ گودام سے نکل چکے ہیں۔",
        whereGoingNext: "اب کہاں جا رہے ہیں؟",

        // Timeline
        tripTimeline: "سفر کا ٹائم لائن",
        steps: "اقدامات",
        next: "اگلا",

        // Months
        january: "جنوری",
        february: "فروری",
        march: "مارچ",
        april: "اپریل",
        may: "مئی",
        june: "جون",
        july: "جولائی",
        august: "اگست",
        september: "ستمبر",
        october: "اکتوبر",
        november: "نومبر",
        december: "دسمبر",

        // Trip History
        loadingTrips: "سفر لوڈ ہو رہے ہیں...",
        noTripsFound: "کوئی سفر نہیں ملا",
        noTripsMessage: "آپ نے کوئی سفر مکمل نہیں کیا",
        viewDetails: "تفصیلات دیکھیں",
        hideDetails: "تفصیلات چھپائیں",
        timeline: "ٹائم لائن",
        warehouses: "گودام",
        warehouse: "گودام",
        duration: "دورانیہ",
        completed: "مکمل",

        // Errors & Warnings
        locationDenied: "مقام کی اجازت مسترد۔ براہ کرم GPS فعال کریں۔",
        locationUnavailable: "مقام دستیاب نہیں۔ GPS سگنل چیک کریں۔",
        failedToLogState: "حالت لاگ کرنے میں ناکام۔ یقینی بنائیں کہ GPS فعال ہے۔",
        locationWarning: "⚠️ مقام کی اجازت درکار ہے۔ براہ کرم اپنے براؤزر کی ترتیبات چیک کریں۔",
        failedToStartTrip: "سفر شروع کرنے میں ناکام۔ براہ کرم دوبارہ کوشش کریں۔",

        // Misc
        pinnedLocation: "پین کردہ مقام",
        loadingTripState: "سفر کی حالت لوڈ ہو رہی ہے...",
        fuelCapacity: "فیول ٹینک کی گنجائش (لیٹر)",
        editCar: "گاڑی میں ترمیم کریں",
        failedSaveCar: "گاڑی محفوظ کرنے میں ناکام۔ پلیٹ نمبر پہلے سے موجود ہو سکتا ہے۔",
    },
    hi: {
        // Header
        driverPanel: "ड्राइवर पैनल",
        logout: "लॉग आउट",
        idle: "निष्क्रिय",
        trip: "यात्रा",

        // Tabs
        activeTrip: "सक्रिय यात्रा",
        tripHistory: "यात्रा इतिहास",

        // Active Trip
        startNewTrip: "नई यात्रा शुरू करें",
        starting: "शुरू हो रहा है...",
        processing: "प्रक्रिया हो रही है...",
        tripCompleted: "यात्रा पूर्ण!",

        // States
        exitFactory: "फ़ैक्टरी से निकास",
        arriveWarehouse: "गोदाम आगमन",
        exitWarehouse: "गोदाम से निकास",
        arriveFactory: "फ़ैक्टरी आगमन",
        // Direct DB mappings
        "Exit Factory": "फ़ैक्टरी से निकास",
        "Arrival at Warehouse": "गोदाम आगमन",
        "Exit Warehouse": "गोदाम से निकास",
        "Arrival at Factory": "फ़ैक्टरी आगमन",
        EXIT_FACTORY: "फ़ैक्टरी से निकास",
        ARRIVE_WAREHOUSE: "गोदाम आगमन",
        EXIT_WAREHOUSE: "गोदाम से निकास",
        ARRIVE_FACTORY: "फ़ैक्टरी आगमन",

        // Actions
        logExitFactory: "फ़ैक्टरी निकास लॉग करें",
        logArriveWarehouse: "गोदाम आगमन लॉग करें",
        logExitWarehouse: "गोदाम निकास लॉग करें",
        logArriveFactoryEnd: "फ़ैक्टरी आगमन लॉग करें (यात्रा समाप्त)",
        goToAnotherWarehouse: "दूसरे गोदाम पर जाएं",
        returnToFactory: "फ़ैक्टरी वापस जाएं",
        warehouseExitQuestion: "आप गोदाम से निकल चुके हैं।",
        whereGoingNext: "अब कहां जा रहे हैं?",

        // Timeline
        tripTimeline: "यात्रा समयरेखा",
        steps: "चरण",
        next: "अगला",

        // Months
        january: "जनवरी",
        february: "फ़रवरी",
        march: "मार्च",
        april: "अप्रैल",
        may: "मई",
        june: "जून",
        july: "जुलाई",
        august: "अगस्त",
        september: "सितंबर",
        october: "अक्टूबर",
        november: "नवंबर",
        december: "दिसंबर",

        // Trip History
        loadingTrips: "यात्राएं लोड हो रही हैं...",
        noTripsFound: "कोई यात्रा नहीं मिली",
        noTripsMessage: "आपने कोई यात्रा पूरी नहीं की",
        viewDetails: "विवरण देखें",
        hideDetails: "विवरण छिपाएं",
        timeline: "समयरेखा",
        warehouses: "गोदाम",
        warehouse: "गोदाम",
        duration: "अवधि",
        completed: "पूर्ण",

        // Errors & Warnings
        locationDenied: "स्थान अनुमति अस्वीकृत। कृपया GPS सक्षम करें।",
        locationUnavailable: "स्थान अनुपलब्ध। GPS सिग्नल जांचें।",
        failedToLogState: "स्थिति लॉग करने में विफल। सुनिश्चित करें कि GPS सक्षम है।",
        locationWarning: "⚠️ स्थान अनुमति आवश्यक है। कृपया अपनी ब्राउज़र सेटिंग जांचें।",
        failedToStartTrip: "यात्रा शुरू करने में विफल। कृपया पुन: प्रयास करें।",

        // Misc
        pinnedLocation: "पिन किया गया स्थान",
        loadingTripState: "यात्रा स्थिति लोड हो रही है...",
    }
};
